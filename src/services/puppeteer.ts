...
async function getBilibiliMediaInfo(page: Page) {
  ...
  const poster = await page.$$eval(
    ...
    (elements) =>
      (elements.find((el) => el.textContent?.trim() === "下载封面") || {}).href
  );
  const url = await page.$$eval(
    "[data-testid='flowbite-card'] a",
    (elements) =>
      (elements.find((el) => el.textContent?.trim() === "下载视频") || {}).href
  );
...
async function getYoutubeMediaInfo(page: Page) {
  const name = await page.$eval(
  ...
  const audio = await page.$$eval(
    "[data-testid='flowbite-card'] a",
    (elements) =>
      (elements.find((el) => el.textContent?.trim() === "下载音频") || {}).href
  );
  const video = await page.$$eval(
    ...
    (elements) =>
      (
        elements.find((el) =>
          el.textContent?.trim().match(/\d{3,}P（mp4）/i)
        ) || {}
      ).href
  );
  return {
